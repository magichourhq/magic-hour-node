  ...
  describe("Integration and edge cases", () => {
    test("should handle multiple file uploads sequentially", async () => {
      ...
      for (let i = 0; i < files.length; i++) {
          ...
          items: [
            {
              ...
              filePath: `api-assets/${i}/${file.path.split("/").pop()}`,
              expiresAt: "2024-12-31T23:59:59Z",
            },
          ],
        };
        ...
        expect(result).toBe(`api-assets/${i}/${file.path.split("/").pop()}`);
        expect(client.v1.files.uploadUrls.create).toHaveBeenCalledWith({
    ...
    test("should handle concurrent file uploads", async () => {
      const files = [
        { path: "/path/to/image1.jpg", type: "image", ext: "jpg", id: "1" },
        { path: "/path/to/image2.png", type: "image", ext: "png", id: "2" },
        { path: "/path/to/video1.mp4", type: "video", ext: "mp4", id: "3" },
      ];
          ...
          items: [
            {
              ...
              filePath: `api-assets/${file.id}/${file.path.split("/").pop()}`,
              expiresAt: "2024-12-31T23:59:59Z",
            },
          ],
        };
      ...
      const uploadPromises = files.map((file) =>
        client.v1.files.uploadFile(file.path),
      );
      ...
      files.forEach((file, index) => {
        expect(results[index]).toBe(
          `api-assets/${file.id}/${file.path.split("/").pop()}`,
        );
      });
    });
