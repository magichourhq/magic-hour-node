  ...
  test.concurrent(
    ...
    async () => {
      ...
      const [rawResponse, response] = await Promise.all([
        client.v1.imageProjects.delete({ id: "string" }).asResponse(),
        client.v1.imageProjects.delete({ id: "string" }),
      ]);
...
describe("tests client.v1.imageProjects.get", () => {
  test.concurrent(
    ...
    async () => {
      ...
      // Get both raw response for status and parsed response for data
      const [rawResponse, response] = await Promise.all([
        client.v1.imageProjects.get({ id: "string" }).asResponse(),
        client.v1.imageProjects.get({ id: "string" }),
      ]);
